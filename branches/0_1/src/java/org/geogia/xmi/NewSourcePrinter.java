...
public class NewSourcePrinter extends VisitorAdapter {
     ...
     */
    public NewSourcePrinter(PrintStream out,String[] tokenNames) {
     ...
     */
    public NewSourcePrinter(PrintStream out,String[] tokenNames, boolean newLines) {
        ...
        if (lastLinePrinted != currentLine) {
            if (newLines) {
                if (!(visit == OPENING_VISIT && t.getType() == GroovyTokenTypes.SLIST)) {
                	int j=0;
                    for (int i=lastLinePrinted;i<currentLine;i++,j++) {
                        out.println();
                        if(j==1) break;
                    }
                    if (lastLinePrinted > currentLine) {
